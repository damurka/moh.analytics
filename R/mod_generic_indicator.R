...
#'
#' @noRd
mod_generic_indicator_ui <- function(id, var_name, icon, target_val, unit) {
  ns <- NS(id)

  target <- if (is.na(target_val)) {
    HTML("&ndash;")
  } else {
    paste(target_val, unit)
  }
  tagList(
    layout_columns(
      fill = FALSE,
      ...
      value_box(
        str_glue("{var_name}"),
        uiOutput(ns('value'), container = h3),
      ...
      value_box(
        ...
        value = uiOutput(ns("trend"), container = h3),
...
#'
#' @noRd
mod_generic_indicator_server <- function(id, cache, var_cols, target_val, indicator_title, unit){
  ...
  moduleServer(id, function(input, output, session){
    ...
    output$value <- renderUI({
      ...
      val_col <- if (type == 'adjusted') {
        var_cols[grep('_adj', var_cols)]
      } else {
        var_cols[!grep('_adj', var_cols)]
      }
      paste(scales::number(summarised_data()[[val_col]]), ' ', unit)
    })

    output$trend <- renderUI({
      req(cache())
      if (is.null(summarised_data()) || nrow(summarised_data()) == 0 || is.na(target())) {
        return(HTML("&ndash;"))
      }
      ...
      paste(scales::number(val), unit)
    })

    output$map <- renderEcharts4r({
      req(county_data(), cache())
    ...
    output$plot <- renderEcharts4r({
      ...
      monthly_data() %>%
        mutate(across(any_of(c(unadj_col, adj_col)), round, 0)) %>%
        e_charts(month) %>%
        e_line_(unadj_col, name = 'Unadjusted', smooth = FALSE) %>%
        e_line_(adj_col, name = 'Adjusted', smooth = FALSE) %>%
        e_y_axis(min = 0, max = round(ceiling(ymax / 10) * 10, 0), digits = 0) %>%
        e_mark_line(data = list(yAxis = target()),
                    lineStyle = list(color = "red", type = "dashed"),
                    label = list(formatter = str_glue("Target: {target()} {unit}"))) %>%
        e_labels() %>%
        e_theme("macarons") %>%
        e_title(title, textStyle = list(fontSize = 16), left = 10, right = 60) %>%
        e_legend(orient = "horizontal", bottom = 0, left = "center", selected = list('Unadjusted' = type == 'unadjusted', 'Adjusted' = type == 'adjusted')) %>%
